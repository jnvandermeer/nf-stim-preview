...
fd_with_markers = visualHelper.convert_to_fd_vis(FPS=60.,VIS_SHOWOPPOSITE=False)
...
if __name__=="__main__":
    ...
    testinstr=[visual.TextStim(win, TEXT[0],pos=(0, 0.5)), visual.TextStim(win, TEXT[1],pos=(0, 0.5))]
        ...
        while not correctlyPressed:
            ...
            if len(evs) > 0:
                ...
                if buttonsPressed[0] == BUTTONS[i]:
                    ...
                    incorr_str = visual.TextStim(win, INCORRECT_TEXT,pos=(0, -0.5))
                    drawIncorrect=True
                    incor_clock.reset()
                

    contstim=visual.TextStim(win, 'Press to continue...',pos=(0.0, 0.90), height=0.08)
    startstim=visual.TextStim(win, 'Press to Start!',pos=(0.0, 0.0), height=0.12)
    contstim.draw()
    win.flip()
    event.waitKeys()
    ...
    ecstim = visual.TextStim(win, 'Close your eyes!',pos=(0.0, 0.0), height=0.12)
    ecstim.draw()
    win.flip()
    incor_clock.reset()
    ...
    while incor_clock.getTime()<float(EC_TIME):
        pass
